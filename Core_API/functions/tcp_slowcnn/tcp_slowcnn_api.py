...
from models.models import ConnectToDB, TcpSlowConnection
...
class TcpSlowConnectionCollectionAPI(Resource):
	...
	def get(self,profile_id):
		try:
			...
			limit, page, offset, order = getDefault(
				parameters, TcpSlowConnection.__table__.columns, TcpSlowConnection
			)
			query = session.query(TcpSlowConnection).filter(TcpSlowConnection.profile == profile_id)
			if "search" in parameters and parameters['search'] != "":
				search_values = parameters['search'].split(",")
				for search_value in search_values:
					query = query.filter(or_(key.like('%'+ search_value +'%')\
						for key in TcpSlowConnection.__table__.columns))
...
class TcpSlowConnectionAPI(Resource):

	def get(self,profile_id, tcp_slowcn_id):
		try:
			session = Session()
			parameters = request.args
			
			record = session.query(TcpSlowConnection).filter(TcpSlowConnection.profile == profile_id).filter_by(id = tcp_slowcn_id).one()
	...
	def put(self, profile_id,tcp_slowcn_id):
		try:
			session = Session()
			parameters = request.args
			...
			if check_null(data) is False:
				return status_code_400("Request Data Error!")
			if "data" in data:
				data['data'] = ','.join(str(i) for i in data['data'])
			session.query(TcpSlowConnection).filter(TcpSlowConnection.profile == profile_id).filter(TcpSlowConnection.id == tcp_slowcn_id).update(data)
			try:
				session.commit()
			except Exception as exp:
				print (exp)
