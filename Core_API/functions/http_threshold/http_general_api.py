...
from models.models import ConnectToDB, HttpGeneral
...
class HttpGeneralCollectionAPI(Resource):
	...
	def get(self,profile_id):
		try:
			...
			limit, page, offset, order = getDefault(
				parameters, HttpGeneral.__table__.columns, HttpGeneral
			)
			query = session.query(HttpGeneral).filter(HttpGeneral.profile == profile_id)
			if "search" in parameters and parameters['search'] != "":
				search_values = parameters['search'].split(",")
				for search_value in search_values:
					query = query.filter(or_(key.like('%'+ search_value +'%')\
						for key in HttpGeneral.__table__.columns))
...
class HttpGeneralAPI(Resource):

	def get(self, profile_id,http_general_id):
		try:
			session = Session()
			parameters = request.args
			
			record = session.query(HttpGeneral).filter(HttpGeneral.profile == profile_id).filter_by(id = http_general_id).one()
	...
	def put(self,profile_id, http_general_id):
		try:
			session = Session()
			parameters = request.args
			...
			session.query(HttpGeneral).filter(HttpGeneral.profile == profile_id).filter(HttpGeneral.id == http_general_id).update(data)
			try:
				session.commit()
			except Exception as exp:
				print (exp)
