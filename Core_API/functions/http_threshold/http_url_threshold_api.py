...
class HttpUrlThresholdCollectionAPI(Resource):
    ...
    def get(self,profile_id):
        try:
            ...
            for i in range(len(records)):
                urls = []
                for url in records[i].http_threshold_value:
...
class HttpUrlThresholdAPI(Resource):

    def get(self,profile_id, url_threshold_id):
        try:
            session = Session()
            parameters = request.args
            
            record = session.query(HttpThreshold).filter(HttpThreshold.profile == profile_id).\
                filter(HttpThreshold.type == "url").filter_by(id = url_threshold_id).one()
    ...
    def put(self, profile_id,url_threshold_id):
        try:
            session = Session()
            parameters = request.args
            ...
            if "urls" in data:
                session.query(HttpThresholdValue).\
                    filter(HttpThresholdValue.http_threshold_id == url_threshold_id).delete()
                try:
                    session.commit()
                except Exception as exp:
                    print (exp)
                ...
                for url in data['urls']:
                    http_threshold_value = HttpThresholdValue(
                        http_threshold_id = url_threshold_id,
                        value = url
                    )
            ...
            session.query(HttpThreshold).\
                filter(HttpThreshold.profile == profile_id).filter(HttpThreshold.type == "url").\
                filter(HttpThreshold.id == url_threshold_id).update(data)
            try:
                session.commit()
            except Exception as exp:
                print (exp)
                session.rollback()
                return status_code_500("")
            return status_code_200("Edit HTTP url Threshold Success!", "")
        except Exception as exp:
            print (exp)
            return status_code_500("")
        finally:
    ...
    def delete(self,profile_id, url_threshold_id):
        try:
            session = Session()
            parameters = request.args
            ...
            session.query(HttpThreshold).filter(HttpThreshold.profile == profile_id).\
                filter(HttpThreshold.type == "url").\
                filter(HttpThreshold.id == url_threshold_id).delete()
            try:
                session.commit()
            except Exception as exp:
                print (exp)
                session.rollback()
                return status_code_500("")
            return status_code_200("Delete HTTP url Threshold Success!", "")
        except Exception as exp:
